...
def chat():
    ...
    # store girlfriend
    if m := re.match(r'^(?:Moje přítelkyně se jmenuje)\s+(.+)$', msg, re.IGNORECASE):
    ...
    if re.search(r'^Jak se jmenuji\?*$', msg, re.IGNORECASE):
        return jsonify({'reply': kv.get('name', 'Nevím.')})
    if re.search(r'^Co mám rád\?*$', msg, re.IGNORECASE):
        return jsonify({'reply': ', '.join(likes) or 'Nevím.'})
